...
//        collection parameters.
    ...
    URI getTapServiceURI() {
        return applicationConfiguration.lookupServiceURI(AdvancedRunner.TAP_SERVICE_URI_PROPERTY_KEY,
                                                         AdvancedRunner.DEFAULT_TAP_SERVICE_URI);
    }
    ...
    private void handleError(final Throwable throwable,
        ...
        final ErrorSummary errorSummary = new ErrorSummary(throwable.getMessage(), errorType);
        try {
            ...
            LOGGER.error("failed to set final error status after " + throwable, e);
    ...
    private boolean hasUploadError() {
        ...
        final String parameter = RegexParameterUtil.findParameterValue(UploadResults.UPLOAD_ERROR_COUNT,
                                                                       job.getParameterList());

        try {
            return StringUtil.hasText(parameter) && (Integer.parseInt(parameter) > 0);
        } catch (NumberFormatException e) {
            LOGGER.error("Unable to parse parameter to Integer " + UploadResults.UPLOAD_ERROR_COUNT);
            return false;
        }
    }
     ...
     */
    private String generateErrorVOTable() {
        return "<?xml version=\"1.0\"?>"
                + "<VOTABLE xmlns=\"http://www.ivoa.net/xml/VOTable/v1.2\" "
                + "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
                + "version=\"1.2\">"
                + "<INFO name=\"EXTRA_INFO\" value=\"Uploaded file invalid.\"/>"
                + "</VOTABLE>";
    }
